...
import { setMonthlySavings } from "@/api/endpoints";
...
/**
 * This month's savings goal on the tracker. "On track" means the income you
 * haven't spent yet still covers the whole goal; once spending eats into it,
 * the shortfall is shown. Editing here writes the same savingsByMonth target
 * that drives the daily budget and the streak.
 */
export default function SavingsGoalCard({
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await setMonthlySavings({ key: `${year}-${month}`, amount });
      await refresh();
      setOpen(false);
      toast.success(
        amount > 0
          ? `Savings goal set for ${monthName(month)}`
          : `Savings goal cleared for ${monthName(month)}`
      );
      onUpdated?.();
    } catch (err) {
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="font-semibold">
                  No savings goal for {monthName(month)}
                </p>
                <p className="text-xs text-muted-foreground">
        ...
        <div className="space-y-4 pb-2">
          <div className="space-y-2">
            <Label htmlFor="tracker-savings">
              Amount to set aside in {monthName(month)} {year}
            </Label>
            <Input
              id="tracker-savings"
              type="number"
          ...
          <Button className="w-full" onClick={handleSave} disabled={saving}>
            {saving ? "Saving…" : `Save for ${monthName(month)}`}
          </Button>
        </div>
      </BottomSheet>
    </motion.div>
